...
router.get('/captcha', function (req, res) {
  var captcha = svgCaptcha.create({
    ignoreChars: '0o1l',  // 验证码字符中排除 0o1i
    noise: 2,    //干扰线条的数量
    color: true    // 验证码的字符是否有颜色，默认没有，如果设定了背景，则默认有
  });
  req.session.captcha = captcha.text.toLowerCase();  // 验证码文字
  console.log(req.session.captcha,'+++++++++=');
  res.type('svg');
  res.send(captcha.data)   //svg 路径
});
...
router.get('/sendcode', function (req, res, next) {
  ...
  //发送给指定的手机号
  var code = '1234';
  // console.log(`向${phone}发送验证码短信: ${code} 哈哈`);
  sms_util.sendCode(phone,function (success) { //success表示是否成功
    if (success) {
      res.send({"code": 0})
    } else {
      //3. 返回响应数据
